...
> This solves infinite gambling and gives incentive to use charged items more by not being so expensive to repair (with the exception of Teleport).
...
- Clone Frost Nova to a new Chance to Cast (CTC) Frost Nova skill which has 100-150 damage per skill level.
...
> Removing ED% and flat damage from armors as it caused the "ED bug".

- Removed 5-10 weapon damage from Reign of Deceit.
- Removed 0.25 max damage/level from Ancients' Epiphany.
- Added +2 random Paladin skills to Ancients' Epiphany.
- Bonemesh unique Chains boots changed from Bone Mesh to Myrmidon Greaves.
- Thoqqua's Slipper (Scarabshell Boots): Required level fixed from 19 to 49 (base boots requirement).
- Wild Horses (Myrmidon Greaves): Required level fixed from 34 to 65 (base boots requirement).
...
### Physical Jewels
> Physical jewels all had %ED and flat damage which caused the "ED bug" in non-weapons. Adjustments were made.
...
#### Star Facet
- Now has 45-50% Enhanced Damage, 15% Increased Attack Speed, and 5 to all stats.

### Poison Jewels
> Removed the small amount of flat poison from the two poison facets.

### The Saboteur's Bandolier (Renamed from Medusa's Gaze)
> Duplicate entry for Medusa's Gaze reworked to be a +Max Traps belt.
...
### Judge's Helmet (Renamed from Treachery's Allure)
...
> The start of an effort to revisit some vanilla sets and provide some love. For Tal's, keeping the theme of providing general elemental damage allowing for hybrid Sorceress builds or just being useful for any build. Should now act as a useful mid/early endgame set. A few additional stats were added; the helm now also has set bonuses.

### Treasure Hunter
> Didn't feel like it had real oomph for being a Magic Find (MF) set. Now fills the niche of anyone wanting to MF with the vanilla MF items as a set.

### Immortal King
> Given a few numerical increases to the items, a few random Barbarian skill rolls, as well as a set-specific skill and buff: King's Trinity and King's Charisma.
>
> **King's Trinity**: Creates a cold/fire/lightning tornado nova on strike. Initial strike is physical, and each elemental missile is converted from 37% of weapon damage to that element.
>
> **King's Charisma**: Provides a flat +1000-2000 cold/fire/lightning damage to attacks.

### Trang-Oul's Avatar
> Reworked Trang's to be a true fire-wielding vampire set.

- Now has its own unique oskills:
    - **Trang-Oul's Burning Shroud**: "Conjures a veil of burning death that devours the living as it advances."
    - **Trang-Oul's Doomfall**: "Call down a burning omen of ruin that erupts in necrofire on impact."
    - **Trang-Oul's Hellflare**: "Unleash a barrage of scorching orbs that explode into fiery death on impact."
